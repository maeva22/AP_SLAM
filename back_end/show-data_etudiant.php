...
$stmt = $db_etudiant->prepare(
    "SELECT ENTREPRISE.ID_ENTREPRISE, NOM_ENTREPRISE,VILLE_ENTREPRISE, NOM_SALARIE, 
        ...
        FROM salarie 
        INNER JOIN demarche ON DEMARCHE.ID_SALARIE=SALARIE.ID_SALARIE
        INNER JOIN entreprise ON ENTREPRISE.ID_ENTREPRISE= SALARIE.ID_ENTREPRISE
        INNER JOIN moyencom ON MOYENCOM.ID_MOYEN=DEMARCHE.ID_MOYEN
        WHERE DEMARCHE.ID_ETUDIANT=:id 
...
$stmt = $db_etudiant->prepare(
        "SELECT ID_STAGE,ENTREPRISE.ID_ENTREPRISE,DATE_FIN,DATE_DEBUT,NOM_ENTREPRISE,
      	...
      	WHERE STAGE.ID_ETUDIANT=:id AND
            	ETAT ='OK';"
);
